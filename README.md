...
- Build config files: `build/shadow-cljs.edn` and `build/deps.edn`
- Backend is Azure Functions app in `src/backend/api_host/`
- Backend source is ClojureScript: `src/backend/api.cljs`
- Azure entrypoint is `src/backend/api_host/story_routes_host.js` (loads generated `src/dist/backend/backend_api_compiled.js`)
...
```bash
cp src/backend/api_host/local.settings.json.example src/backend/api_host/local.settings.json
...
npm install
npm run build:backend
mkdir -p .deploy/api_host
rsync -a --delete --exclude 'local.settings.json' --exclude 'deploy.zip' --exclude '.deploy.zip' src/backend/api_host/ .deploy/api_host/
cp -R node_modules .deploy/api_host/node_modules
rsync -a --delete defintions/ .deploy/api_host/defintions/
mkdir -p src/dist/backend
(cd .deploy/api_host && zip -rq ../../src/dist/backend/.deploy.zip .)
az functionapp deployment source config-zip -g $RG -n $APP --src src/dist/backend/.deploy.zip
```
...
./workflows/deployment_scripts/deploy_azure.sh robogene-rg eastus robogene-func-prod
```
...
export AZURE_CONFIG_DIR=/Users/penpa/Desktop/PDFs/robogene/.azure
export OPENAI_API_KEY="<your_key>"
./workflows/deployment_scripts/deploy_azure_consumption.sh robogene-rg eastus robogene-func-prod
```
