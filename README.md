...
- Webapp build output: `dist/webapp/js/main.js` (generated by build/CI)
...
- Azure entrypoint is `src/api_host/story_routes_host.js` (loads generated `dist/webapi/webapi_compiled.js`)
...
Build services (ClojureScript -> Azure Functions JS):
...
```bash
./run.robogene.local.sh
```

This starts:
- Webapp + services ClojureScript watcher (`shadow-cljs watch webapp webapi`)
...
Note:
- `npm run build:webapi` is defined in the repo root `package.json` and is the only services build command.
...
npm run dist:webapi
# Produces:
# - dist/webapi/app/            (fully runnable Function App folder)
# - dist/webapi/webapi_dist.zip (zip of dist/webapi/app)
az functionapp deployment source config-zip -g $RG -n $APP --src dist/webapi/webapi_dist.zip
```
...
## Notes
- This services stores story state in-memory per function instance.
...
- `dist/webapi/app/` is the assembled runnable Function App folder used by CI and local deploy scripts.
- `dist/webapp/` build artifacts are not committed; CI builds them for Pages.
