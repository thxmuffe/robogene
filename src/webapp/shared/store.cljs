...
(defn update-command-entity [db payload]
  (let [entity-id (str (:id payload))
        ...
        role (or (some-> (:type payload) str str/lower-case)
                 (:vanityRole existing)
                 "generic")
        title (if (contains? payload :name)
                (:name payload)
                (:title existing))
        description (if (contains? payload :description)
                      (:description payload)
