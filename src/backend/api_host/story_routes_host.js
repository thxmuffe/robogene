...
try {
  try {
    require('./dist/backend_api_compiled.js');
  } catch (_localErr) {
    // Local dev build output is centralized in src/dist/backend.
    require('../../dist/backend/backend_api_compiled.js');
  }
} catch (err) {
  console.error('[robogene] Failed to load backend compiled bundle from api_host/dist or src/dist/backend.');
  console.error(err);
