...
(cd build && npx shadow-cljs release backend >/dev/null)

PKG_DIR="$REPO_ROOT/.deploy/api_host"
rm -rf "$PKG_DIR"
mkdir -p "$PKG_DIR"
rsync -a --delete --exclude 'local.settings.json' --exclude 'deploy.zip' --exclude '.deploy.zip' "$REPO_ROOT/src/backend/api_host/" "$PKG_DIR/"
cp -R "$REPO_ROOT/node_modules" "$PKG_DIR/node_modules"
mkdir -p "$PKG_DIR/dist"
cp "$REPO_ROOT/src/dist/backend/backend_api_compiled.js" "$PKG_DIR/dist/backend_api_compiled.js"
rsync -a --delete "$REPO_ROOT/defintions/" "$PKG_DIR/defintions/"
mkdir -p "$REPO_ROOT/src/dist/backend"
rm -f "$REPO_ROOT/src/dist/backend/.deploy.zip"
(cd "$PKG_DIR" && zip -rq "$REPO_ROOT/src/dist/backend/.deploy.zip" .)
az functionapp deployment source config-zip -g "$RG" -n "$APP" --src "$REPO_ROOT/src/dist/backend/.deploy.zip" >/dev/null
